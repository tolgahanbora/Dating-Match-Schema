...
You'll need to supply a mock auth token and an associated id.

```bash
> curl -X POST https://api.coupling.app/auth --data '{"bareer_token": _token, "_id": user_id}'
```
...
```bash
curl 'https://api.coupling.app/user/matches/{_id} --data '{"message": "your message here"}'
```
...
> curl 'https://api.coupling.app/user/ping --data '{"lat": latitude, "lon": longitude}'
...
### Get "updates"
```bash
> curl 'https://api.coupling.app/updates'
```
...
```json
{
	"matches": [{
		...
		"messages": [{
			...
			"sent_date": "2023-10T08:06:49.800Z",
...
```json
{
    ...
    "results": [{
        ...
        "gender": 1,
        "name": "Cristina",
        "ping_time": "2023-04-06T16:52:51.605Z",
        "photos": [{
